...
- **Bad release shipped:** the in-container updater auto-rolls-back if `/api/health` fails for 120s. For irreversible damage, cut `vX.Y.Z+1` immediately with the fix.
- **"The update always falls back to the version I installed by hand":** the health probe could not reach the new container, so the orchestrator rolled back — the update machinery worked exactly as designed on an endpoint that can never answer. Two records to ask for, both introduced with the fix (first release after v0.42.0): `update-orchestrator.log` in the install's data dir (the orchestrator container auto-removes, this file is what survives it) and the `<container>-failed` container a rollback now leaves behind (`docker logs <container>-failed` — that one holds the *new version's* own boot output, if it ever got that far). Before that fix an install whose container sits on a **user-defined (Compose) network** could never pass the probe: Docker drops traffic between two bridge networks and the orchestrator ran on the default bridge. That is every Synology **Container Manager "project"** and every `docker compose up` install, which is why the reports came from users while the `docker run` NAS install updated fine. Such an install cannot self-update onto the fix — the broken updater is the thing being fixed, and the **drop-to-update bundle takes the same orchestrator path**, so it fails identically. Those users must update the image once by hand (`docker compose pull && docker compose up -d`, or Container Manager ▸ project ▸ Build/Reset), after which in-app updates work.
