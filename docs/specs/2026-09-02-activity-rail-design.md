...
- **Status bar:** its **own small toggle at the very foot of the rail** (window-with-bottom-bar glyph, lit while the bar shows), persisted separately. The status bar is diagnostics more than workspace. That preference is honoured only in the rail layout — the legacy strip has nothing to un-hide it with, so it always shows the bar. There is deliberately **no separate "hide panel" button** on the rail: it would double the click-the-active-icon gesture (a bottom Hide chevron was tried on 2026-09-07 and removed the same day for exactly that reason).
- **Hide (area) — "nothing but the board":** the toolbar `≡` hides panel, rail *and* status bar together (revised 2026-09-07 after the first build: when working on a board you often want nothing else on screen, and 46px of rail is still something). The floating edge arrow, which the rail had made redundant, returns only in this state; it, `≡` again, or any deep link (`showSidebarTab`) restores both. Persisted separately (`boardripper-sidebar-rail-hidden`).
