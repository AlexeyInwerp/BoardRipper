...
- **The toolbar sidebar button cycles** open → icons only → completely hidden → open (revised 2026-09-07, fourth pass; it was a two-state ≡ before). Its glyph is the sidebar-collapse icon, flipping to "expand" when the next click brings things back. The last stage is "nothing but the board": panel, rail *and* status bar, the status bar folded in without touching the foot toggle's own preference. The floating edge arrow returns only in that stage and jumps straight back to open, as does any deep link (`showSidebarTab`). Persisted separately (`boardripper-sidebar-rail-hidden`).
