...
- **Hide (panel):** click the active rail item, the rail's bottom hide chevron, the rail's right-click menu, or the keyboard shortcut. Dragging the width down clamps at 200px rather than hiding — hiding is easy to do by accident with a drag, so it stays a click.
- **Hide (area) — "nothing but the board":** the toolbar `≡` hides panel *and* rail together (revised 2026-09-07 after the first build: when working on a board you often want nothing else on screen, and 46px of rail is still something). The floating edge arrow, which the rail had made redundant, returns only in this state; it, `≡` again, or any deep link (`showSidebarTab`) restores both. Persisted separately (`boardripper-sidebar-rail-hidden`).
