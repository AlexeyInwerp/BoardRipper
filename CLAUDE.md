...
- **Cloud-storage-aware file serving (v0.20.4+):** the three file-serve handlers (`databank.PreviewGet`, `files.Get`, `files.GetByPath`) read files fully into memory via `serveFileEager` (`src/backend/handlers/serve.go`) and verify byte count matches `stat().Size()` before writing the response. Truncated reads from cloud-sync placeholders (Google Drive on macOS File Provider, OneDrive on Windows NTFS reparse points, iCloud, Dropbox) produce a 503 with `Retry-After: 5` instead of corrupt bytes; the 30s read deadline produces 503 with `Retry-After: 10`. Frontend `fetchWithCloudRetry` (`src/frontend/src/store/fetch-with-cloud-retry.ts`) retries up to 6 attempts / 3 min, surfaces a "Downloading from cloud storage…" toast on retry, and surfaces an error toast on exhaustion. Trade-offs: loses range-request and ETag support (no consumer needed them); 512 MiB cap on in-memory reads.
