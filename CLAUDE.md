...
- **PDF canvas rules:** All opaque PDF canvases use `getContext('2d', { alpha: false })` to skip alpha compositing. Canvas pool (8 entries) shrinks backing store synchronously before pooling — never defer shrinking (race condition with reuse). Highlight and glyph canvases retain alpha.
