...
func (e *PdfExtractor) ExtractAll(concurrency int) (extracted, errors int) {
	...
	log.Printf("PdfExtractor: %d PDFs to extract", len(toExtract))
...
func (e *PdfExtractor) ExtractOne(file FileRecord) error {
	absPath := filepath.Join(e.dataDir, file.Path)
...
func extractPdfText(path string) ([]string, error) {
	...
	for i := 1; i <= numPages; i++ {
		...
		if err != nil {
			log.Printf("PdfExtractor: page %d text extraction failed: %v", i, err)
			continue
		}
		pages[i-1] = text
	}
...
func (db *DB) HasPdfText(fileID int64) (bool, error) {
	var count int
	err := db.conn.QueryRow(`SELECT COUNT(*) FROM pdf_pages WHERE file_id = ?`, fileID).Scan(&count)
	return count > 0, err
}
...
func (db *DB) InsertPdfPage(fileID int64, pageNum int, text, source string) error {
	...
	_, _ = db.conn.Exec(`DELETE FROM pdf_text WHERE file_id = ? AND page_num = ?`, fileID, pageNum)
...
func (db *DB) DeletePdfText(fileID int64) error {
	...
	if _, err := db.conn.Exec(`DELETE FROM pdf_text WHERE file_id = ?`, fileID); err != nil {
		return err
	}
	_, err := db.conn.Exec(`DELETE FROM pdf_pages WHERE file_id = ?`, fileID)
	return err
}
...
func (db *DB) BatchExtractStatus() (map[int64]bool, error) {
	rows, err := db.conn.Query(`SELECT DISTINCT file_id FROM pdf_pages`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
