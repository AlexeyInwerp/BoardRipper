...
| `build/pdf.worker.mjs` | top of file (block 1) | Worker-context shims for the ES2024/2025 APIs pdfjs-dist@5 calls directly: `Promise.try` (RESOLVE/STREAM/PULL/CANCEL message dispatch — Chrome 128+), `Promise.withResolvers` (capability objects across PDFManager / stream sinks — Chrome 119+), `URL.parse` (annotation link + font-fetch base resolution — Chrome 120+), `ArrayBuffer.prototype.transferToFixedLength` (font-substitution write buffers — Chrome 129+), `Uint8Array.fromBase64` (XFA `$content` payloads — Chrome 140+). Main-thread polyfills in `src/polyfills.ts` don't cross the Worker boundary, so this block mirrors them inside the worker. Pinpoint trigger for the Legacy Mac build (Electron 22 / Chromium 108). Idempotent. |
| `build/pdf.worker.mjs` | top of file (block 2) | Worker-context shims for `Uint8Array.prototype.toHex` and `Map.prototype.getOrInsertComputed`. pdf.js 5.5 calls both directly (`fingerprints` getter, font caches, AcroForm, XFA — 6 call sites total). Main-thread polyfills in `pdf-store.ts` don't cross the Worker boundary, so older Chromium/Firefox/Safari and legacy Electron forks crash with `hashOriginal.toHex is not a function` on PDF open. Idempotent (`typeof !== 'function'` guard) so we don't shadow native impls on Chrome 136+. |
