...
export const SETTINGS_INDEX: IndexEntry[] = [
  ...
  F('board', 'parts', 'showComponentColors', 'Type colours (show)', 'Fill component bodies with colors based on their type prefix (R = resistor, C = capacitor, U = IC, etc.). Colors are configured in Part Type Overrides'),
  F('board', 'parts', 'componentFillAlpha', 'Type colours (fill opacity)', 'Transparency of the component type color fills. 0 = invisible, 1 = fully opaque'),
  F('board', 'parts', 'partLabelScale', 'Part names (size)', 'Multiplier on the auto-fit designator size; the same row shows or hides the names', ['label', 'designator', 'size']),
  F('board', 'parts', 'showPartLabels', 'Part names (show)', 'Display component reference designators (e.g. U1, R100, C42) centered on each part', ['refdes', 'designator', 'text', 'font']),
  ...
  F('board', 'pins', 'pinNumberScale', 'Pin numbers (size)', 'Multiplier on the pin-number text size; the same row shows or hides the numbers', ['label', 'size']),
  F('board', 'pins', 'showPinNumbers', 'Pin numbers (show)', 'Display pin number/name labels inside pin circles on multi-pin components (ICs, connectors). On BGA parts, numbers and net names alternate vertically to reduce overlap'),
  F('board', 'pins', 'netLabelScale', 'Net names (size)', 'Multiplier on the net-name text size on pins; the same row shows or hides the names', ['label', 'size']),
  F('board', 'pins', 'showNetNames', 'Net names (show)', 'Display the net name on each pin (GND and NC are excluded — already colour-coded). Turn off to unclutter dense parts, or to leave the pin free for diode readings', ['net', 'net name', 'label', 'signal', 'declutter']),
  F('board', 'pins', 'diodeValueScale', 'Diode values (size)', 'Multiplier on the on-pin diode reading size; the same row shows or hides the readings', ['diode', 'size']),
  F('board', 'pins', 'showDiodeValues', 'Diode values (show)', 'Draw diode-mode reference readings on pins (XZZ-baked values + OpenBoardData). Only visible on boards that carry readings; also in the board sidebar View tab', ['diode', 'voltage', 'reading', 'obd', 'measurement']),
  F('board', 'pins', 'diodeValuesOnly', 'Diode Values Only', 'While diode values are shown, hide pin numbers and net names board-wide so a pin carries nothing but its reading. Your Show Pin Numbers / Show Net Names settings are left untouched. Same as the third state of the diode button on the board overlay', ['diode', 'only', 'declutter', 'diode mode', 'reading']),
  ...
  F('board', 'netLines', 'netLineDashed', 'Dashed (on/off)', 'Draw net connection lines as dashed instead of solid. Easier to distinguish from board traces'),
  F('board', 'netLines', 'netLineDashLength', 'Dashed (dash length)', 'Length of each dash segment (screen pixels) in the dashed net line pattern'),
